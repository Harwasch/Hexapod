...
STEP = 300.0
PROPORTIONS = [
    ("250 / 500 (2.0×) — agreed", sprawl_ypp(femur=250, tibia=500, femur_deg=45, key="p1")),
    ("250 / 625 (2.5×) — earlier", sprawl_ypp(femur=250, tibia=625, femur_deg=55, key="p2")),
...
def fig_torques():
    ...
    ax.set_xticks(x)
    short = {"sprawl": "Sprawl YPP\nfemur 45° (agreed)", "sprawl_narrow": "Sprawl YPP\nfemur 65°, feet in",
             "sprawl_mammal": "Same leg,\nmammal stance", "mammal": "Mammal RPP\nfeet under", "lizard": "Sprawl RYP\nroll hip"}
    ...
    ax2.set_xticks(x)
    ax2.set_xticklabels([short.get(k, k) for k in keys], fontsize=7, rotation=30, ha="right")
...
def write_doc(figs):
    ...
    S, M, L, MM = RESULTS["sprawl"], RESULTS["mammal"], RESULTS["lizard"], RESULTS["sprawl_mammal"]
...
* **Sprawl, yaw–pitch–pitch (A, agreed).** Nothing on the yaw joint but
  ...
  margin, highest body.
* **Same leg with the femur at 65° (feet in).** Lower femur torque, {RESULTS['sprawl_narrow']['stance_width']:.2f} m
  stance, and the fold limit bites harder — {RESULTS['sprawl_narrow']['extreme']['reach_fraction']*100:.0f} % of the step-up box.
  It shows the trade inside the chosen topology: the femur angle is a
  posture the controller can choose, not a build decision.
* **Same leg in its mammal stance (yaw 90°).** Hips {MM['topo'].hip_height:.0f} mm up, feet under
  the hips, {MM['stance_width']:.2f} m stance, the whole step-up box reachable. The yaw joint
  goes almost idle ({MM['routine']['neutral'][0]:.0f} N·m) and the pitch joints take the propulsion over
  the full leg height: femur {MM['routine']['max'][1]:.0f} and knee {MM['routine']['max'][2]:.0f} N·m in the routine volume,
...
the 625 mm tibia first chosen that was {STEPS[1][2]['e_min']:.0f} mm, so a foot that had to
...
The agreed 2.0× leg stands on a 300 mm step with the foot only {STEPS[0][2]['outward']:+.0f} mm
outward at {STEPS[0][2]['femur_step']:.0f} N·m on the femur, from a {STEPS[0][2]['hip']:.0f} mm hip; the 2.5× leg needed
{STEPS[1][2]['outward']:+.0f} mm and {STEPS[1][2]['femur_step']:.0f} N·m from a {STEPS[1][2]['hip']:.0f} mm hip. The height the shorter tibia gives up in
the sprawl stance comes back in the mammal stance ({MM['topo'].hip_height:.0f} mm hips), which is
